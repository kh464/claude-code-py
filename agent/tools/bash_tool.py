...
class BashTool(ToolDef):
    metadata = ToolMetadata(name="BashTool")
    input_schema = {
        "type": "object",
        "properties": {
            "command": {"type": "string"},
    ...
    def validate_input(self, args: Mapping[str, Any]) -> None:
        ...
        assert_command_safe(command, shell="bash")
    ...
    async def call(
        ...
        cwd = (
            str(Path(str(workdir_input)).expanduser().resolve())
            if workdir_input is not None
            else None
        )
        ...
        try:
            ...
            raise ToolInterruptedError("Bash tool execution interrupted") from exc
